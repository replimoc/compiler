...
\node[fill=kit!10,above=of coloring]	(outsource) { Outsource };
...
\begin{tikzpicture}
\node[fill=kit!10]			(outsource) { Outsource };
\node[fill=kit!10,right=of outsource]	(coloring) { Color Graph };
\node[fill=kit!10,right=of coloring]	(ssa) { SSA-Deconstruction };
...
\visible<5-> {\item Peephole-Optimizations: Eliminate unneccessary jumps}
\end{itemize}
\end{frame}
